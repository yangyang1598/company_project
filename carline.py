...
def Yolo(img, score, nms):
    ...
    for i in range(len(boxes)):
        ...
        if i in indexes:
            ...
            cv2.rectangle(img, (x, y - 19), (int(x + len(text) * 2 * 4.5), y - 10 + len(text) * 2), (255, 200, 0), -1)
...
while True:
    ...
    #img = my.Undistort(img, 'company.p')
    ...
    img_temp = cut_img(img_src, src, dst)
    img_perspect = cut_img(img, src, dst)
    temp = np.zeros_like(img)


    img_yolo1=img[int(0.5*height):int(0.95*height),int(0.46*width):int(0.53*width),:].copy()
    #img[h,w,channel]순서
    img_yolo=np.zeros_like(img)
    img_yolo[int(0.5*height):int(0.95*height),int(0.46*width):int(0.53*width),:]=img_yolo1
    cv2.imshow("a",img_yolo)
    img_yolo,yolo_mid_y=Yolo(img_yolo, 0.1, 0.4)
    #cv2.imshow("yolo",img_yolo)
    img_yolo=make_dark(img_yolo,120)
    # cv2.imshow("yolo1", img_yolo)
    ...
    #cv2.imshow("white", img_white_line)
    ...
    car_dis=height-yolo_mid_y
    if car_dis==1080:
        car_dis=0


    img_result = cv2.addWeighted(img, 1., temp, 0.4, 0)
    img_result=cv2.addWeighted(img_result,1.,img_yolo,1,0)
    if error > 0:
        ...
        cv2.putText(img_result, f'center', (width // 2, height // 2), cv2.FONT_ITALIC, 0.5,
                    (0, 0, 255), 2)
    if car_dis!=0:
        cv2.putText(img_result, f'car_distace is {car_dis/10} cm', (width//2 - 270, height - 20), cv2.FONT_ITALIC, 1,
                (255, 255, 255), 2)



    _, result = my.imgsize(img_result, 1920)
